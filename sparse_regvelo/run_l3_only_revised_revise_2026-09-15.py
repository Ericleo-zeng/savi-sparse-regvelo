...
def run_l3_streaming(ms_path: str,
    ...
    # >>> 2026-09-15 新增（T0.5 加固）：溯源记录。06_compare_conditions 对比时会
    # 检查 ref/test 的 l3_metadata 键差异——source_checkpoint 块使"双方来自不同
    # L2 拟合来源"（含 hash 级证据）从不可见变为可见。
    meta_out["source_checkpoint"] = {
        "path": os.path.abspath(checkpoint_dir),
        "beta_sha256": _file_sha256(os.path.join(checkpoint_dir, "beta.npy")),
        "gamma_sha256": _file_sha256(os.path.join(checkpoint_dir, "gamma.npy")),
        "metadata_sha256": _file_sha256(os.path.join(checkpoint_dir, "metadata.json")),
        "W_sha256": _file_sha256(os.path.join(checkpoint_dir, "W.npz")),  # V6: W 是实验变量，必须可溯源
        "rate_max_requested": meta.get("rate_max_requested"),
