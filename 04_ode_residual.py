...
输入：
    production L2 checkpoint（参数）+ production L3 输出（stored 解）+ Ms/Mu（初值）
输出：
    results/V1_ode_residual/V1_result.json
    results/V1_ode_residual/residual_hist.png
...
def main() -> int:
    ...
    out_root = os.path.join(C.RESULTS_DIR, "V1_ode_residual")
    os.makedirs(out_root, exist_ok=True)

    # ---- 加载 production checkpoint 参数
    import torch
    ...
    # ---- 抽样：初值来自 production Ms/Mu
    rng = np.random.default_rng(C.RANDOM_SEED + 1)
    idx = np.sort(rng.choice(C.EXPECTED_N, size=args.n_cells, replace=False))
    Ms = np.load(C.MS_PATH, mmap_mode="r")
    Mu = np.load(C.MU_PATH, mmap_mode="r")
    s0 = np.asarray(Ms[idx], dtype=np.float32)
    u0 = np.asarray(Mu[idx], dtype=np.float32)

    # ---- 尝试取 production stored 解；缺失则用 production 参数现场重算
    ...
    try:
        ...
        if os.path.isfile(s_path) and os.path.isfile(u_path):
            ...
            print(f"[V1] 使用 production stored 解: {l3_dir}")
        else:
        ...
        print(f"[V1] production stored 解不可用（{e}）")
        print(f"[V1] 退化模式：现场以 production 参数 (K={K_prod}, tol={PICARD_TOL}, "
    ...
    # ---- production 自身日志信息一并纳入（若存在）
    try:
        l3_meta_p = os.path.join(C.find_l3_output(), "l3_metadata.json")
